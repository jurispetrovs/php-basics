...
$allLicenses = ['P', 'S']; //P = pistol, S = shotgun
...
$person[0]->licenses = [$allLicenses[0]]; //Pistol license
...
$guns[0]->name = 'Pistol';
$guns[0]->license = $allLicenses[0]; //Pistol license
$guns[0]->cost = 134.89;
...
foreach ($person as $eachPerson) {
    ...
    if ($licenseCount <= 0) {
        echo 'Sorry ' . $eachPerson->name . ", don't have any gun licenses" . PHP_EOL;
    } else {
        for ($i = 0; $i < count($eachPerson->licenses); $i++) {
            foreach ($guns as $eachGun) {
                if ($eachPerson->licenses[$i] === $eachGun->license && $eachPerson->money >= $eachGun->cost) {
                    echo $eachPerson->name . ' have ' . $eachGun->name . ' gun license, and can buy it by '
                        . $eachGun->cost . '$' . PHP_EOL;
                } elseif ($eachPerson->money < $eachGun->cost) {
                    echo $eachPerson->name . ' have ' . $eachGun->name . " gun license, but can't buy it by "
                        . $eachGun->cost . '$, because he have only ' . $eachPerson->money . '$' . PHP_EOL;
                }
            }
        }
    }
